...
use frame_support::traits::{{Contains, Get}, tokens::SystemTokenId};
...
pub struct AsPrefixedOriginalSystemTokenId<Prefix, ParaId, AssetId, ConvertAssetId, L = MultiLocation>(
...
impl<
	...
	L: SystemTokenId<AssetId=u128, PalletId=u8, OriginId=u32> + TryInto<MultiLocation> + TryFrom<MultiLocation> + Clone
> MaybeEquivalence<L, AssetId> for AsPrefixedOriginalSystemTokenId<Prefix, ParaId, AssetId, ConvertAssetId, L> {
	...
	fn convert_back(what: &AssetId) -> Option<L> {
		...
		let mut pallet_id = match latest_prefix.interior {
			Junctions::X1(PalletInstance(id)) => {
				Some(id)
			},
			_ => None,
		};
