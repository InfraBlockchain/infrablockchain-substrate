...
pub(crate) const fn div(a: F, b: F) -> F {
	...
	let written_exponent: i32 = (a_exponent.wrapping_sub(b_exponent).wrapping_add(scale as u32))
	...
	let abs_result = if written_exponent > 0 {
		...
		residual = (a_significand
			.wrapping_shl(significand_bits.wrapping_add(written_exponent as u32)) as u32)
...
mod test {
	#[test]
	fn sanity_check() {
		assert_eq!(f32!(10.0).div(f32!(5.0)), f32!(2.0))
	}
}
