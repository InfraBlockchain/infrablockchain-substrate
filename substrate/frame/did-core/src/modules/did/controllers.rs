...
impl<T: Config> Pallet<T> {
	pub(crate) fn add_controllers_(
		AddControllers { did, controllers, .. }: AddControllers<T>,
		OnChainDidDetails { active_controllers, .. }: &mut OnChainDidDetails,
	) -> DispatchResult {
		for ctrl in &controllers {
			ensure!(!Self::is_controller(&did, ctrl), Error::<T>::ControllerIsAlreadyAdded)
		}
	...
	pub(crate) fn remove_controllers_(
		RemoveControllers { did, controllers, .. }: RemoveControllers<T>,
		OnChainDidDetails { active_controllers, .. }: &mut OnChainDidDetails,
	) -> DispatchResult {
		for controller_did in &controllers {
			ensure!(Self::is_controller(&did, controller_did), Error::<T>::NoControllerForDid)
		}
	...
	pub fn ensure_controller(controlled: &Did, controller: &Controller) -> Result<(), Error<T>> {
		ensure!(Self::is_controller(controlled, controller), Error::<T>::OnlyControllerCanUpdate);
