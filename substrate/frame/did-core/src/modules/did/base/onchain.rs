...
impl<T: TypesAndLimits> TryFrom<StoredDidDetails<T>> for StoredOnChainDidDetails<T> {
	...
	fn try_from(details: StoredDidDetails<T>) -> Result<Self, Self::Error> {
		details.into_onchain().ok_or(Error::<T>::CannotGetDetailForOnChainDid)
	}
}
...
impl OnChainDidDetails {
	...
	pub fn new(last_key_id: IncId, active_controller_keys: u32, active_controllers: u32) -> Self {
		Self { last_key_id, active_controller_keys, active_controllers }
	}
}
...
impl<T: Config> Pallet<T> {
	...
	pub fn onchain_did_details(did: &Did) -> Result<StoredOnChainDidDetails<T>, Error<T>> {
		Self::did(did).ok_or(Error::<T>::DidDoesNotExist)?.try_into()
	}
}
