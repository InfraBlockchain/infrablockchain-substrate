...
fn offchain_did() {
	...
	ext().execute_with(|| {
		...
		assert!(vec![129; 300].try_into().map(OffChainDidDocRef::<Test>::Custom).is_err());

		// Add a DID
		assert_ok!(DIDModule::new_offchain(Origin::signed(alice), did, doc_ref.clone()));
...
fn add_keys_to_did() {
	ext().execute_with(|| {
		...
		for pk in vec![PublicKey::sr25519(pk_sr), PublicKey::ed25519(pk_ed), pk_secp] {
			for vr in vec![
				VerRelType::AUTHENTICATION,
				VerRelType::ASSERTION,
				VerRelType::CAPABILITY_INVOCATION,
...
fn becoming_controller() {
	...
	ext().execute_with(|| {
		...
		let add_keys = AddKeys {
			did: did_2,
			keys: vec![UncheckedDidKey::new(pk_secp, VerRelType::CAPABILITY_INVOCATION)],
			nonce: 6 + 1,
		};
		let sig = SigValue::sr25519(&add_keys.to_state_change().encode(), &pair_sr);
		assert_ok!(DIDModule::add_keys(
	...
	ext().execute_with(|| {
		...
		assert_noop!(
			DIDModule::add_service_endpoint(
				...
				DidSignature { did: Controller(did), key_id: 1u32.into(), sig }
			),
			Error::<Test>::OnlyControllerCanUpdate
		);
		...
		assert_noop!(
			DIDModule::add_service_endpoint(
				Origin::signed(alice),
				add_service_endpoint,
				DidSignature { did: Controller(did), key_id: 2u32.into(), sig }
			),
			Error::<Test>::InsufficientVerificationRelationship
		);

		assert!(vec![20; 512].try_into().map(ServiceEndpointId::<Test>).is_err()); // too big id not allowed
		assert!(vec![30; 561].try_into().map(ServiceEndpointOrigin::<Test>).is_err()); // too big origin not allowed
		assert!(vec![ServiceEndpointOrigin::<Test>(vec![30; 20].try_into().unwrap()); 300]
			.try_into()
			.map(|origins| ServiceEndpoint { types: ServiceEndpointType::LINKED_DOMAINS, origins })
			.is_err()); // too many origins not allowed
		...
		for (id, ep) in vec![
			...
			let add_service_endpoint = AddServiceEndpoint { did, id, endpoint: ep, nonce: 5 + 1 };
			let sig = SigValue::sr25519(&add_service_endpoint.to_state_change().encode(), &pair_sr);
		...
		let rem_service_endpoint =
			RemoveServiceEndpoint { did, id: endpoint_1_id.clone(), nonce: 7 + 1 };
		...
		let rem_service_endpoint = RemoveServiceEndpoint { did, id: endpoint_1_id, nonce: 8 + 1 };
		let sig = SigValue::sr25519(&rem_service_endpoint.to_state_change().encode(), &pair_sr);
		assert_noop!(
			DIDModule::remove_service_endpoint(
				Origin::signed(alice),
		...
		let rem_service_endpoint =
			RemoveServiceEndpoint { did, id: endpoint_2_id.clone(), nonce: 8 + 1 };
		let sig = SigValue::sr25519(&rem_service_endpoint.to_state_change().encode(), &pair_sr);
