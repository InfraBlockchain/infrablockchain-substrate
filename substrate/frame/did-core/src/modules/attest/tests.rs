...
fn encoded_attestation_size() {
	ext().execute_with(|| {
		...
		.cloned()
		{
			assert_eq!(Attestation::<Test> { priority, iri }.encode().len(), expected_size);
		}
	});
}
...
fn priority_face_off() {
	ext().execute_with(|| {
		...
		set_claim(
			&did,
			&Attestation { priority: 2, iri: Some(vec![0].try_into().unwrap()) },
			&kp,
			11 + 1,
		)
		.unwrap();
		...
		assert_eq!(
			set_claim(
				&did,
				&Attestation { priority: 2, iri: Some(vec![0, 2, 3].try_into().unwrap()) },
				&kp,
				12 + 1
			)
			.unwrap_err(),
...
fn priority_battle_royale() {
	ext().execute_with(|| {
		...
		assert_eq!(Attestations::<Test>::get(did).priority, prios.iter().max().unwrap().clone());
	});
}
...
fn set_some_attestation() {
	ext().execute_with(|| {
		...
		let (did, kp) = newdid();
		let did = Attester(did);
		assert_eq!(Attestations::<Test>::get(did), Attestation { priority: 0, iri: None });
		check_nonce(&did, 10);
		set_claim(
			&did,
			&Attestation { priority: 1, iri: Some(vec![0, 1, 2].try_into().unwrap()) },
			&kp,
			10 + 1,
		)
		.unwrap();
