	...
	fn notauthorized() {
		...
		fn assert_revoke_err(
			...
			let rev = RevokeRaw {
				_marker: PhantomData,
				registry_id: regid,
				revoke_ids: random::<[[u8; 32]; 32]>().iter().cloned().map(Into::into).collect(),
			};
...
mod calls {
	...
	fn unrevoke() {
		...
		let cases: &[(Action, &[RevokeId], u32)] = &[
			...
			(Action::UnRevo, &[RevokeId(random()), RevokeId(random())], line!()),
			(
				Action::UnRevo,
				&[RevokeId(random()), RevokeId(random()), RevokeId(random())],
				line!(),
...
mod test {
	...
	fn ensure_auth() {
		...
		for (i, (line_no, policy, signers, expect_success)) in cases.iter().enumerate() {
			...
			Registries::<Test>::insert(RGA, Registry { policy: policy.clone(), add_only: false });
