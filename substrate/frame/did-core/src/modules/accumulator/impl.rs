...
impl<T: Config> Pallet<T> {
	...
	pub(super) fn remove_params_(
		RemoveAccumulatorParams { params_ref: (did, counter), .. }: RemoveAccumulatorParams<T>,
		owner: AccumulatorOwner,
	) -> DispatchResult {
		// Only the DID that added the param can remove it
		ensure!(did == owner, Error::<T>::NotAccumulatorOwner);
		ensure!(AccumulatorParams::<T>::contains_key(did, counter), Error::<T>::ParamsDontExist);
	...
	pub(super) fn remove_public_key_(
		RemoveAccumulatorPublicKey { key_ref: (did, counter), .. }: RemoveAccumulatorPublicKey<T>,
		owner: AccumulatorOwner,
	) -> DispatchResult {
		ensure!(did == owner, Error::<T>::NotAccumulatorOwner);
		ensure!(AccumulatorKeys::<T>::contains_key(did, counter), Error::<T>::PublicKeyDoesntExist);
	...
	pub(super) fn add_accumulator_(
		AddAccumulator { id, accumulator, .. }: AddAccumulator<T>,
		owner: AccumulatorOwner,
	) -> DispatchResult {
		ensure!(!Accumulators::<T>::contains_key(id), Error::<T>::AccumulatorAlreadyExists);
		...
		Accumulators::<T>::insert(id, AccumulatorWithUpdateInfo::new(accumulator, current_block));
	...
	pub(super) fn update_accumulator_(
		UpdateAccumulator { id, new_accumulated, .. }: UpdateAccumulator<T>,
		owner: AccumulatorOwner,
	) -> DispatchResult {
		Accumulators::<T>::try_mutate(id, |accumulator| -> DispatchResult {
			let accumulator = accumulator.as_mut().ok_or(Error::<T>::AccumulatorDoesntExist)?;

			// Only the DID that added the accumulator can update it
			ensure!(*accumulator.accumulator.owner_did() == owner, Error::<T>::NotAccumulatorOwner);
	...
	pub(super) fn remove_accumulator_(
		...
		ensure!(*accumulator.accumulator.owner_did() == signer, Error::<T>::NotAccumulatorOwner);
