...
fn create_blob(
	...
	let bl = Blob { id, blob: content.clone().try_into().ok()? };
	println!("did: {:?}", author);
...
fn add_blob() {
	fn add(size: usize, block_no: u64) {
		...
		create_blob(id, noise.clone(), BlobOwner(author), author_kp, block_no + 1)
			.unwrap()
			.unwrap();
		// Can retrieve a valid blob and the blob contents and author match the given ones.
		assert_eq!(Blobs::<Test>::get(id), Some((BlobOwner(author), noise.try_into().unwrap())));
...
fn err_blob_already_exists() {
	ext().execute_with(|| {
		...
		create_blob(id, random_bytes(10), BlobOwner(author), author_kp.clone(), 10 + 1)
			.unwrap()
			.unwrap();
