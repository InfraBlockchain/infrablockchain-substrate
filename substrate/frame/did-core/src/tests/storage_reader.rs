...
const DUMMY_CTX: evm::Context =
	evm::Context { address: H160([0; 20]), caller: H160([0; 20]), apparent_value: U256([32; 4]) };

macro_rules! assert_decoded_eq {
	($expr: expr, $val: expr) => {{
		assert_eq!(
			$expr.map_err(ErrorWrapper::Execution).and_then(
				|PrecompileOutput { output, exit_status, .. }| {
...
fn map_access() {
	ext().execute_with(|| {
		...
		assert_decoded_eq!(
			MetaStorageReader::<Test>::execute(&mut MockHandle::new(
				input.with_replaced_key(MapKey::new_single(non_existent_did)).encode(),
				Some(10_000_000),
				DUMMY_CTX
			)),
			Ok(None::<()>)
